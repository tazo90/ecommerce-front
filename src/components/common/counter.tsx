...
export default function Counter({
  ...
  function getButtonClasses({ direction }: { direction: "l" | "r" }) {
    return cn(
      "flex items-center justify-center flex-shrink-0 h-full transition ease-in-out duration-300 focus:outline-none",
      {
        [`w-10 md:w-12 text-heading border-${direction} border-gray-300 hover:text-white hover:bg-heading`]:
          variant === "default",
  ...
  return (
    ...
    >
      <button
        onClick={onDecrement}
        className={getButtonClasses({ direction: "r" })}
      ...
      <button
        onClick={onIncrement}
        className={getButtonClasses({ direction: "l" })}
